...
This plugin processes one or two CUDLL BAM files and creates a single merged FASTQ file containing primary reads.
...
- Skips reads without sequence information, since they cannot be emitted as FASTQ
...
2. Writes all eligible primary reads from the supplemental BAM to the output FASTQ with formatted names
3. Writes eligible primary reads from the main BAM that don't appear in the supplemental BAM
4. Formats read names as `cell_barcode^umi^read_name` using CB and UMI tags
5. Skips reads that lack sequence information needed for FASTQ output
...
- Takes a main BAM file, an optional supplemental BAM, and a sample name as input
...
- Only reads with populated sequence and quality fields can be written to FASTQ
